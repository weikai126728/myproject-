...
function member_del(obj){
	...
	layer.confirm('确认要彻底删除吗？',function(index){
		$.post('${pageContext.request.contextPath}/manager/evaluate/del.do',{id:id},function(data){
			if(data.code==0){
				if(data.result==true){
...
function pass(obj){
	...
	layer.confirm('确认要恢复数据吗？',function(index){
		$.post('${pageContext.request.contextPath}/manager/evaluate/recovery.do',{id:id},function(data){
			if(data.code==0){
				if(data.result==true){
					layer.msg('恢复已数据!',{icon:1,time:1000},function(){
						oTable1.fnClearTable();
					});
				}
			}
...
var oTable1;
jQuery(function($) {
	var url = '${pageContext.request.contextPath}/manager/evaluate/dustbinList.do';
			...
			'ajax':{
				...
				'dataSrc':function(data){
					for(var i=0;i<data.result.length;i++){
						...
						var st = '<span class="label label-warning radius">已删除</span>';
						var handle = '<a onClick="pass(this)"  href="javascript:;" title="恢复"  class="btn btn-xs btn-success"><i class="fa fa-check  bigger-120"></i></a>'
							+'<a  onClick="Guestbook_iew(this);" title="回复"  href="javascript:;"  class="btn btn-xs btn-info" ><i class="fa fa-edit bigger-120"></i></a>'     
