...
var search;
jQuery(function($) {
	var url = '${pageContext.request.contextPath}/manager/order/findByPage.do';
			...
			'dataSrc':function(data){
				for(var i=0;i<data.result.length;i++){
					...
					if(data.result[i].status>-1){
						handle+='<a title="取消订单" id="'+data.result[i].id+'" customerId="'+data.result[i].customerId+'"  href="javascript:void(0);" onclick="cancelOrder(this);"  class="btn btn-xs btn-info order_detailed" ><i class="fa fa-times bigger-120"></i></a> ';						
					}
					var image = '';
...
function excel(){
	...
	location.href = '${pageContext.request.contextPath}/manager/order/excel.do?createTime='+startTime+'&search='+search;
}
</script>
</html>