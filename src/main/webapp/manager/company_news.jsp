		...
		<div id="Member_Ratings">
			<div class="d_Confirm_Order_style">
				...
				<div class="border clearfix">
					<span class="l_f"> <a
						href="${pageContext.request.contextPath}/manager/company_newsAdd.jsp"
						title="添加商品" class="btn btn-warning Order_form"><i
							class="icon-plus"></i>发布新闻</a>
...
<script>
	...
	jQuery(function($) {
		var url = '${pageContext.request.contextPath}/company/news/findByType.do';
		oTable1 = $('#sample-table').dataTable({
							"processing" : true,
							"serverSide" : true,
							'ajax' : {'url' : url,'data' : function(d) {d.typeId = typeId;},
								'dataSrc' : function(data) {
									for (var i = 0; i < data.result.length; i++) {
										...
										var handle = '<a title="删除" href="javascript:;"  id="'+data.result[i].id+'" onclick="member_del(this)" class="btn btn-xs btn-warning" ><i class=" bigger-120">删除</i></a>'
												+ '<a title="修改" href="javascript:;" id="'+data.result[i].id+'"  onclick="member_edit(this)" class="btn btn-xs btn-info" ><i class=" bigger-120">修改</i></a>'
												+ '<a title="预览" href="javascript:;" id="'+data.result[i].id+'"  onclick="member_show(this)" class="btn btn-xs btn-success" ><i class=" bigger-120">预览</i></a>';
										data.result[i].handle = handle;
										data.result[i].type = type;
									}
									return data.result;
								}
							},
	...
	function member_del(obj, id) {
		layer.confirm('确认要删除吗？',function(index) {
			$.post("${pageContext.request.contextPath}/company/news/deleteById.do",{id : obj.id}, function(data) {
				if (data.result == true) {
	...
	function member_show(obj) {
		url = '${pageContext.request.contextPath}/company/news/findById.do?id='
				+ obj.id;
		layer_show(obj.title, url);
	}
	function member_edit(obj) {
		url = '${pageContext.request.contextPath}/company/news/editById.do?id='
				+ obj.id;
		layer_show(obj.title, url);
	}
	laydate({
