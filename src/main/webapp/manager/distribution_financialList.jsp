						...
						<tbody>
							<%-- <c:forEach items="${financialList }" var="financial">
								<tr>
									...
									<td id="id">${financial.id }</td>
									<td>${financial.user.name }</td>
									<c:if test="${financial.type==0 }">
										<td>扣款</td>
									</c:if>
									<c:if test="${financial.type==1 }">
										<td>入款</td>
									</c:if>
...
jQuery(function($) {
		var url = '${pageContext.request.contextPath}/distribution/financial/findByType.do';
	...
	function selectUser(w, h) {
		var url = '${pageContext.request.contextPath}/distribution/financial/selectFinancialBy.do?name='
				+ $("#seleName").val();
		var title = '搜索结果';
