...
jQuery(function($) {
		...
		var url = '${pageContext.request.contextPath}/redpack/queryFzvoruserList.do';
		...
		'ajax':{
			...
			'dataSrc':function(data){
				...
				for(var i=0;i<data.result.length;i++){
					...
					if(data.result[i].tag==1){
						data.result[i].tag='已发放';
					var handle ='<a title="已发放" data="'+data.result[i].openid+'" onclick=" " href="javascript:;"  class="btn btn-xs btn-info" ><i class="icon-edit bigger-120">已发放</i></a>';
					data.result[i].handle = handle;
					}else{
						data.result[i].tag='未发放';
					var handle ='<a title="发放" data="'+data.result[i].openid+'" onclick="member_edit(this)" href="javascript:;"  class="btn btn-xs btn-info" ><i class="icon-edit bigger-120">发放</i></a>';
					data.result[i].handle = handle;
					}
				}
				return data.result;
...
function member_edit(obj){
	...
	var url = '${pageContext.request.contextPath}/redpack/addUserActivities.do?openid='+openid;
	location.href = url;
	//layer_show("编辑",url);
}
