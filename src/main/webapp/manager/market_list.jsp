					...
					<table class="table table-striped table-bordered table-hover"
						...
						<thead>
							<tr>
								...
								<th width="100">联系人</th>
								<th width="120">手机</th>
								<th width="120">店铺编码</th>
								<th width="100">超市地址</th>
								<th width="180">加入时间</th>
								<th width="100">广告金余额</th>
								<th width="100">广告金消耗</th>
								<th width="70">状态</th>
								<th width="550">操作</th>
							</tr>
						</thead>
							 ...
							 <c:forEach items="${marketList }" var="market">
								<tr>
									...
									<td><u style="cursor: pointer" class="text-primary"
										onclick="member_show('${market.name }','${pageContext.request.contextPath}/manager/market/marketShow.do?id=${market.id }','500','400')">${market.name }
									</u></td>
									<td>${market.contactUser }</td>
									<td>${market.phone }</td>
									<td>${market.tel }</td>
									<td>${market.address.busRoute }</td>
									<td>${market.createTime }</td>
									<td>${market.balance }</td>
									<td>${market.consume }</td>
									<c:if test="${market.status==1 }">
										<td class="td-status"><span
											class="label label-success radius">已启用</span></td>
										<td class="td-manage"><a
											onClick="member_stop(this,'${market.id}')"
											href="javascript:;" title="停用" class="btn btn-xs btn-success"><i
												class="icon-ok bigger-120"></i></a> <a title="编辑"
											onclick="member_edit('编辑','${pageContext.request.contextPath}/manager/market/goEdit.do?marketId=${market.id}')"
											href="javascript:;" class="btn btn-xs btn-info"><i
												class="icon-edit bigger-120"></i></a> <a title="删除"
											href="javascript:;" onclick="member_del(this,'${market.id}')"
											class="btn btn-xs btn-warning"><i
												class="icon-trash  bigger-120"></i></a></a>
												<a title="广告金"
											href="javascript:;" onclick="member_show('广告金','${pageContext.request.contextPath}/manager/market/addgold.do?marketId=${market.id}')"
											class="btn btn-xs btn-warning"><i
											...
											class="btn btn-xs btn-warning"><i
												...
												<a title="添加广告"
											href="javascript:;" onclick="member_show('添加广告','${pageContext.request.contextPath}/manager/market/toAddShare.do?marketId=${market.id}')"
											class="btn btn-xs btn-warning"><i
												class="icon-ok  bigger-120"></i></a>
												<a title="添加动态"
											href="javascript:;" onclick="member_show('添加动态','${pageContext.request.contextPath}/manager/market/addVipcn.do?marketId=${market.id}')"
											class="btn btn-xs btn-warning"><i
												class="icon-ok  bigger-120"></i></a>
												<a title="添加积分商品"
											href="javascript:;" onclick="member_show('添加福利商品','${pageContext.request.contextPath}/manager/market/addstorePro.do?marketId=${market.id}')"
											class="btn btn-xs btn-warning"><i
												class="icon-ok  bigger-120"></i></a>
												<a title="添加联盟活动"
											href="javascript:;" onclick="member_show('添加联盟活动','${pageContext.request.contextPath}/manager/benefit/addbenefitPro.do?marketId=${market.id}')"
											class="btn btn-xs btn-warning"><i
												class="icon-ok  bigger-120"></i></a>
												</td>
									</c:if>
									<c:if test="${market.status==0 }">
										...
										<td class="td-manage"><a style="text-decoration: none"
											class="btn btn-xs "
											onClick="member_start(this,'${market.id}')"
											...
											class="btn btn-xs btn-warning"><i
												class="icon-ok  bigger-120"></i></a>
												<a title="添加积分商品"
											href="javascript:;" onclick="member_show('添加积分商品','${pageContext.request.contextPath}/manager/market/addproi.do?marketId=${market.id}')"
											class="btn btn-xs btn-warning"><i
												class="icon-ok  bigger-120"></i></a>
												<a title="添加福利商品"
											href="javascript:;" onclick="member_show('添加福利商品','${pageContext.request.contextPath}/manager/market/addstorePro.do?marketId=${market.id}')"
...
jQuery(function($) {
				...
				.confirm(
						...
						function(index) {
							...
							$
									.post(
											//提交的路径	
											"${pageContext.request.contextPath}/manager/market/stop.do",
											//提交的参数
											{
												marketId : id
											},
				...
				.confirm(
						...
						function(index) {
							...
							$
									.post(
											//提交的路径	
											"${pageContext.request.contextPath}/manager/market/start.do",
											//提交的参数
											{
												marketId : id
											},
	...
	function member_del(obj, id) {
		layer.confirm('确认要删除吗？', function(index) {
			...
			$.post(
			//提交的路径	
			"${pageContext.request.contextPath}/manager/market/deleteMarket.do",
			//提交的参数
			{
				marketId : id
			},
