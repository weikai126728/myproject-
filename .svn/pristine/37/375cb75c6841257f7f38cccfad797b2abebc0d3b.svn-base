      ...
      <thead>
		...
		<c:forEach var="model" items="${modelList}" varStatus="status">
			<tr>
				<td>${model.name }</td>
				<td>${model.details}</td>
				<td id="${model.id}" name="${model.name }" details="${model.details}">
					<a title="修改" href="javascript:edit(${model.id});" class="btn btn-xs btn-info"><i class="fa fa-cog bigger-120"></i></a>
					<a title="删除" href="javascript:del(${model.id});" class="btn btn-xs btn-info"><i class="fa fa-trash bigger-120"></i></a>
				</td>
			</tr>
		</c:forEach>
        </tbody>
...
function edit(id){
	...
	layer.open({
		  ...
		  ,yes: function(index, layero){
		    $.post('${pageContext.request.contextPath}/manager/alliance/model/edit.do',{name:$('#name').val(),details:$('#details').val(),id:id},function(data){
				var msg ='登陆超时！';
		    	if(data.code==0){
					if(data.result==true){
						msg = '修改成功！';
...
function del(id){
	layer.confirm('确认要删除吗？',function(index){
		$.post('${pageContext.request.contextPath}/manager/alliance/model/del.do',{id:id},function(data){
			if(data.code==0){
				if(data.result==true){
 ...
 function add(){
	...
	$('#details').val('');
	 layer.open({
		  ...
		  ,btn: ['确定', '取消']
		  ,yes: function(index, layero){
		    $.post('${pageContext.request.contextPath}/manager/alliance/model/add.do',{name:$('#name').val(),details:$('#details').val()},function(data){
				var msg ='登陆超时！';
		    	if(data.code==0){
					if(data.result==true){
						msg = '新增成功！';
