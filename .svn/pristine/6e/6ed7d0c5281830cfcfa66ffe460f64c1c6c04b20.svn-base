			...
			<div class="d_Confirm_Order_style">
				...
				<div class="border clearfix">
					<span class="l_f"> <a href="javascript:ovid()"
						id="member_add" class="btn btn-warning"><i class="icon-plus"></i>新增用户</a>
...
<script>
	...
	function member_stop(obj, id) {
		layer.confirm('确认要停用吗？',function(index) {
			$.post("${pageContext.request.contextPath}/manager/user/stop.do",{userId : id},function(data) {
				if (data.result == 'true') {
					$(obj).parents("td").prepend('<a style="text-decoration:none" class="btn btn-xs " onClick="member_start(this,'+ id + ')" href="javascript:;" title="启用"><i class=" bigger-120">启用</i></a>');
	...
	function member_start(obj, id) {
		layer.confirm('确认要启用吗？',function(index) {
			$.post("${pageContext.request.contextPath}/manager/user/start.do",{userId : id},function(data) {
				if (data.result == 'true') {
					$(obj).parents("td").prepend('<a style="text-decoration:none" class="btn btn-xs btn-success" onClick="member_stop(this,'+ id + ')" href="javascript:;" title="停用"><i class=" bigger-120">停用</i></a>');
