				...
				<div class="search_style">
					...
					<ul class="search_content clearfix">
						<li><label class="l_f">会员名称</label><input name="name"
							id="seleName" type="text" class="text_add" placeholder="可以搜索会员名、订单号、产品名称"
						...
						<tbody>
							<%-- <c:forEach items="${orderList }" var="order">
								<tr>
									...
									<td class="td-manage"> 
								       <a title="删除" href="javascript:;" onclick="member_del(this,'${order.id}')"
...
jQuery(function($) {
		var url = '${pageContext.request.contextPath}/distribution/order/findByType.do';
			...
			'dataSrc':function(data){
				for(var i=0;i<data.result.length;i++){
					...
					if(data.result[i].status==0){
						st = '<span class="label radius">未付款</span>';
					}
					if(data.result[i].summary=="" || data.result[i].summary== null){
						summary='<td></td>'
					}
					data.result[i].status = st;
					data.result[i].handle = handle;
					data.result[i].summary = summary;
				}
				return data.result;
			}		
		},
	...
	function member_del(obj, id) {
		layer.confirm('确认要删除吗？', function(index) {
			...
			$.post(
			//提交的路径	
			"${pageContext.request.contextPath}/distribution/order/deleteById.do",
			//提交的参数
			{
				orderId : id
			},
	...
	function selectUser(w, h) {
		var url = '${pageContext.request.contextPath}/distribution/order/selectOrderBy.do?name='
				+ $("#seleName").val();
		var title = '搜索结果';
