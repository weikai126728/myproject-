;(function(){
	...
	productData.banners = [];
	var file = document.getElementsByClassName('btn-files1')[0];
	file.onchange = function(e) {
		var files = e.target.files;
		var container = document.getElementById(upload.default.id);
		for(var i = 0; i < files.length; i++) {
			productData.banners.push(files[i]);
			var r = new FileReader();
			r.readAsDataURL(files[i]);
			var that = i;
			r.onload = function(e) {
				var index = productData.banners.length;
				...
				if(window.addEventListener){ // Mozilla, Netscape, Firefox 
					em.addEventListener('click', function(){
						...
						nodes.remove();
						productData.banners.splice(index,1);
				...
				var btn = document.getElementById("btn-img-add1");
...
//	}
	function save(){
		...
		for(var i=0;i<productData.banners.length;i++){
			formData.append('files',productData.banners[i]);
		}
