...
jQuery(function($) {
		...
		var url = '${pageContext.request.contextPath}/redpack/queryVoucherlist.do';
		...
		'ajax':{
			...
			'dataSrc':function(data){
				...
				for(var i=0;i<data.result.length;i++){
					...
					/* var handle ='<a title="编辑" data="'+data.result[i].cadeId+'  class="btn btn-xs btn-info" ><i class="icon-edit bigger-120"></i></a>'   ; 
...
function member_edit(obj,total_fee){
	 var openid = $(obj).attr("data");
	var url = '${pageContext.request.contextPath}/redpack/redpackBufa.do?openid='+openid+'&total_fee='+total_fee;
